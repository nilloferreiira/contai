...
- [x] **Section A: Foundation** (Tasks 1-4) — `2026-09-09-bolso-mvp-implementation/section-a-foundation.md`
  Dependencies/ESLint/root CLAUDE.md, shadcn/ui init + tokens/fonts, QueryProvider + Toaster, vitest setup.
- [x] **Section B: Supabase & Auth** (Tasks 5-8) — `2026-09-09-bolso-mvp-implementation/section-b-supabase-auth.md`
  Supabase project + env + clients, session-refresh middleware, `/login` + `/cadastro`, authenticated route guard + bottom nav shell.
- [ ] **Section C: Database** (Task 9) — `2026-09-09-bolso-mvp-implementation/section-c-database.md`
  `supabase/migrations/0001_init.sql` — full schema, RLS policies, indexes, triggers.
...
- [ ] **Section E: API + hooks** (Tasks 18-24) — `2026-09-09-bolso-mvp-implementation/section-e-api-hooks.md`
  `api-client.ts`/`query-keys.ts`/zod schemas, `/api/cards`, `/api/categories`, `/api/merchants`, `/api/expenses`, `/api/occurrences`, `/api/reports/summary`, plus matching React Query hooks.
...
- [ ] **Section G: Pages** (Tasks 32-35) — `2026-09-09-bolso-mvp-implementation/section-g-pages.md`
  `/inicio`, `/mes`, `/relatorios`, `/ajustes` (+ card/category forms).
